...
#assert(interp-at-1.len() == 1)
#assert.eq(interp-at-1.first().y, 2.0)
// First and last rows are zero-pad.
#assert.eq(r-a.data.first().y, 0)
#assert.eq(r-a.data.first().at("align-padding"), true)
#assert.eq(r-a.data.last().y, 0)
#assert.eq(r-a.data.last().at("align-padding"), true)
// Mid rows are not pad.
#assert.eq(interp-at-1.first().at("align-padding"), false)
...
// Two zero-crossings expected at x = 0.5 and x = 1.5 (linear interp at y=0).
#assert(0.5 in resolved-c.unique-loc)
#assert(1.5 in resolved-c.unique-loc)

#assert.eq(stat-info("align").outputs, ("x", "y", "align-padding"))
