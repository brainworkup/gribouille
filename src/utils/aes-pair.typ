/// Helpers encoding the "exclusive default" rule for geoms that paint both a
/// stroke `colour` and a body `fill`.
///
/// Rule: if the user has set either aesthetic (via `aes()` mapping or as a
...
/// - `(default-colour, none)` when only `colour` is set, suppressing the fill
///   default so the geom does not inject an unwanted body fill;
/// - `(none, default-fill)` when only `fill` is set, suppressing the stroke
///   default so the outline does not appear unsolicited.
///
/// Pass the returned `colour-default` / `fill-default` straight into the
/// existing resolver functions, which already treat a `none` default as
/// "skip the default" (with `apply-alpha` short-circuiting on `none`).
///
/// \@internal
/// \@param layer The layer dictionary providing `params`.
/// \@param mapping The resolved aesthetic mapping handed to a geom's `draw`.
