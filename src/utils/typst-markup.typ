...
/// aesthetic mapping; the scale uses raw values for its mapping logic and
/// evaluates each value as Typst markup wherever the scale renders text
/// (legend swatches, axis ticks, geom labels).
///
/// In static-prose surfaces (titles, captions, axis titles, legend
/// titles), `typst("...")` evaluates the wrapped string as markup.
///
/// `source` may be another tagged value (such as `as-factor("col")`) so
...
///   aesthetic value.
///
/// \@returns A tagged dictionary the renderer recognises as
///   markup-bearing.
///
...
///
/// Returns `true` when `value` is a `typst-markup` tagged dictionary, or
/// when it is a `mapping-ref` whose inner `var` is itself typst-tagged.
/// Plain strings, content, and untagged dictionaries return `false`.
///
/// \@internal
/// \@param value Aesthetic mapping value to inspect.
...
///
/// String values pass through eval; content and other types pass through
/// unchanged. Used at display surfaces to honour a typst-tagged aesthetic
/// once a raw value has been read from a row.
///
/// \@internal
/// \@param value A raw column value (string, int, float, content, etc.).
...
/// - `typst-markup` tagged dictionary whose `source` is itself tagged
///   (composed aes): errors, since static prose has no row context.
///
/// \@internal
