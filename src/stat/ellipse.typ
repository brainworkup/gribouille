...
/// covariance of `(x, y)`.
///
/// Each output row carries the parameters that \@geom-ellipse expects:
/// `(x0, y0, a, b, angle)` with `x0`, `y0` the group centroid, `a`, `b`
/// the semi-major and semi-minor radii in data units, and `angle` the
/// rotation in radians. Pair with `geom-ellipse(stat: stat-ellipse())` or
